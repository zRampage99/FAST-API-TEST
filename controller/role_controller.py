...
def get_by_id(
    ...
    return ApiResponse(
        success=True, 
        message=f"Trovato Ruolo con ID {role.id} successo", 
        data=role, 
...
def get_all(
    ...
    message = "La lista è vuota" if not roles else "Ecco a te la lista dei ruoli"
    return ApiResponse(
        success=True, 
        message=message, 
        data=roles, 
...
def add(
    ...
    return ApiResponse(
        success=True, 
        message="Role creato con successo", 
        data=created_role, 
        token=request.state.new_token
    )
...
def delete_by_id(
    ...
    return ApiResponseEmpty(
        success=True, 
        message=f"Role con ID {role_id} eliminato con successo", 
        token=request.state.new_token
    )